...
import org.example.domain.ws.DP600.DP600ToNPLH.response.CommunicationResponse;
...
public class DPHandler extends SoapHandler {
        ...
        if (communicationStatus.getIsEnable()) {
            ...
            if (communicationStatus.getIsError()) {
                if (communicationStatus.hasErrorText()) {
                    soapResponse = buildSoapEnvelope(CommunicationResponse.FromSoapActionError(soapAction, communicationStatus.getErrorText()).toString(), "dp");
                } else {
                    soapResponse = buildSoapEnvelope(CommunicationResponse.FromSoapActionError(soapAction).toString(), "dp");
                }
            } else
            {
                soapResponse = buildSoapEnvelope(CommunicationResponse.FromSoapActionOk(soapAction).toString(), "dp");
            }
    ...
    protected String buildResponse(HttpExchange exchange, String soapAction) {
        return "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:web=\"http://webservice.dp600.ventana.com/\">" +
                "   <soapenv:Header/>" +
                "   <soapenv:Body>" +
