...
import org.example.domain.ws.COCKPIT.COCKPITToNPLH.response.CommunicationResponse;
...
public class CockpitHandler extends SoapHandler {

    public CockpitHandler(MessageLogger messageLogger, String serverName, WSServer server) {
        ...
        if (communicationStatus.getIsEnable()) {
            ...
            if (communicationStatus.getIsError()) {
                if (communicationStatus.hasErrorText()) {
                    soapResponse = buildSoapEnvelope(CommunicationResponse.FromSoapActionError(soapAction, communicationStatus.getErrorText()).toString(), "cockpit");
                } else {
                    soapResponse = buildSoapEnvelope(CommunicationResponse.FromSoapActionError(soapAction).toString(), "cockpit");
                }
            } else
            {
                soapResponse = buildSoapEnvelope(CommunicationResponse.FromSoapActionOk(soapAction).toString(), "cockpit");
            }
