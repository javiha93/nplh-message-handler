...
var AbstractScope = class {
  ...
  addVariable(identifier, name, selector, exported, src.domain) {
    ...
    if (variable === void 0) {
      ...
      variable.src.domain |= src.domain;
      variable.declarations.push(declaration);
    }
  }
  applyUse(use, variables = this.variables) {
    const variable = variables.get(use.location.text);
    if (variable === void 0 || (variable.src.domain & use.src.domain) === 0) {
      return false;
    }
...
var RootScope = class extends AbstractScope {
  ...
  addVariable(identifier, name, selector, exported, src.domain) {
    if (src.domain & 8 /* Import */) {
      return super.addVariable(identifier, name, selector, exported, src.domain);
    }
    ...
    this.#innerScope.end((variable, key, scope) => {
      ...
      if (namespaceVar === void 0) {
        this.variables.set(key.text, {
          ...
          src.domain: variable.src.domain,
          uses: [...variable.uses]
        });
      } else {
        ...
        namespaceVar.src.domain |= variable.src.domain;
        for (const use of variable.uses) {
...
function mapDeclaration(declaration) {
  return {
    declaration,
    src.domain: getDeclarationDomain(declaration),
    exported: true
  };
}
...
var AbstractNamedExpressionScope = class extends NonRootScope {
  #src.domain;
  #name;
  constructor(name, src.domain, parent) {
    super(parent, 1 /* Function */);
    this.#name = name;
    this.#src.domain = src.domain;
  }
  addUse(use, source) {
    if (source !== this.innerScope) {
      return this.innerScope.addUse(use);
    }
    if (use.src.domain & this.#src.domain && use.location.text === this.#name.text) {
      this.uses.push(use);
    } else {
      return this.parent.addUse(use, this);
    }
...
var UsageWalker = class {
  ...
  #handleDeclaration(node, blockScoped, src.domain) {
    if (node.name !== void 0) {
      this.#scope.addVariable(
        node.name.text,
        node.name,
    ...
    const cb = (node) => {
      ...
      switch (node.kind) {
        ...
        case ts9__default.default.SyntaxKind.Identifier: {
          const src.domain = getUsageDomain(node);
          if (src.domain !== void 0) {
            this.#scope.addUse({ src.domain, location: node });
          }
          return;
        }
      }
